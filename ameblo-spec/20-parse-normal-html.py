...
def _map(arg):
  ...
  for name in names:
    ...
    if '通信エラー' in main:
      continue
    if os.path.exists(f'negative/{ha}') is False:
      open(f'positive/{ha}', 'wb').write( pickle.dumps({'url':url, 'main':main}) )
      print(meta)
      print(main) 

args = {}
for index, name in enumerate(glob.glob('../../../scraping-designs/ameblo-scrape/htmls/*')):
  key = index%16
